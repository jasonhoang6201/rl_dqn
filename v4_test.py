...
NUM_EPISODES          = 1      # V3: more episodes for better convergence
BUFFER_SIZE           = 100
BATCH_SIZE            = 128
GAMMA                 = 0.995
LR                    = 5e-4
...
q_net      = CNNRainbowQNetwork(obs_dim, num_actions, hidden_dim=512).to(DEVICE)
target_net = CNNRainbowQNetwork(obs_dim, num_actions, hidden_dim=512).to(DEVICE)
...
fig.suptitle("Rainbow DQN V4 - 2048", fontsize=14)
ax = axes[0, 0]
...
for i, step in enumerate(rollout[-n_show:], start=len(rollout) - n_show + 1):
    ...
    if step["board"] is not None:
        print(step["board"])
checkpoint_path = "rainbow_dqn_2048_v4.pt"
torch.save({
